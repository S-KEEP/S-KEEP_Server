    ...
    class getUserCategoryList {
        ...
        void 유저_카테고리_리스트_조회에_성공한다() throws Exception {
            ...
            mockMvc.perform(requestBuilder)
                    ...
                    .andExpect(jsonPath("$.userCategoryDtoList").exists())
                    .andExpect(jsonPath("$.userCategoryDtoList.length()").value(6))
                    .andExpect(jsonPath("$.userCategoryDtoList[0].name").value("익사이팅"))
                    .andExpect(jsonPath("$.userCategoryDtoList[0].description").value("Exciting activities"))
                    .andExpect(jsonPath("$.userCategoryDtoList[1].name").value("공원/자연"))
                    .andExpect(jsonPath("$.userCategoryDtoList[1].description").value("Parks and nature-related activities"))
                    .andExpect(jsonPath("$.userCategoryDtoList[2].name").value("휴식"))
                    .andExpect(jsonPath("$.userCategoryDtoList[2].description").value("Relaxation and rest"))
                    .andExpect(jsonPath("$.userCategoryDtoList[3].name").value("역사 및 유적지"))
                    .andExpect(jsonPath("$.userCategoryDtoList[3].description").value("Historical sites and landmarks"))
                    .andExpect(jsonPath("$.userCategoryDtoList[4].name").value("문화/축제"))
                    .andExpect(jsonPath("$.userCategoryDtoList[4].description").value("Cultural events and festivals"))
                    .andExpect(jsonPath("$.userCategoryDtoList[5].name").value("쇼핑/도심"))
                    .andExpect(jsonPath("$.userCategoryDtoList[5].description").value("Shopping and downtown areas"));
        }
    }
